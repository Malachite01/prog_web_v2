window.addEventListener("load", () => {
  ...
  function enableAudioOnce() {
    ...
    // Retirer tous les listeners d'interaction
    document.removeEventListener("click", enableAudioOnce);
    document.removeEventListener("touchend", enableAudioOnce);
  }
  
  if (sessionStorage.getItem("audioUnlocked") !== "true") {
    // Écouter à la fois click (desktop) et touchend (mobile)
    document.addEventListener("click", enableAudioOnce);
    document.addEventListener("touchend", enableAudioOnce);
  }
});
